...
cat $TMP/files | xargs --null -P0 egrep -iIH -v 'eval[[:space:]]+?\([[:space:]]+?(base64|gzinflate|edoced)'
...
cat $TMP/files | xargs --null -P0 egrep -iIH -v 'ex|exec|passthru|system|pcntl_|proc_|posix_|ssh2_|ssh_'
